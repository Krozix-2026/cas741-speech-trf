...
UC & Unlikely Change\\
WER & Word Error Rate\\
\progname & Program name for the Model--to--Brain Alignment for Speech Recognition system\\
\bottomrule
\end{tabular}
...
\item[\refstepcounter{acnum} \actheacnum \label{acAudioFormat}:] The storage
format, indexing scheme, and metadata organization of the audio datasets.

\item[\refstepcounter{acnum} \actheacnum \label{acMEGFormat}:] The storage
format, trial organization, and metadata organization of the MEG datasets.

\item[\refstepcounter{acnum} \actheacnum \label{acParams}:] The set of input
parameters, including file locations, model choices, lag windows, and
...
\item[\refstepcounter{acnum} \actheacnum \label{acFrozen}:] The representation
and loading strategy for trained checkpoints and frozen models.

\item[\refstepcounter{acnum} \actheacnum \label{acRepresentation}:] The method
used to extract hidden states and convert them into aligned predictor matrices.

\item[\refstepcounter{acnum} \actheacnum \label{acAnalysis}:] The statistical
analysis protocol used to fit encoding models, compute cross-validated scores,
and quantify baseline-referenced gains.

\item[\refstepcounter{acnum} \actheacnum \label{acPlot}:] The choice of
visualization routines and output figure formats.
\end{description}
...
R2 (\textit{R-InputValidation}) 
& \mref{mAudio}, \mref{mMEG}, \mref{mParams}, \mref{mExtract}, \mref{mStats} \\

R3 (\textit{R-ConstructPredictors}) 
& \mref{mFrozen}, \mref{mExtract} \\

R4 (\textit{R-FairEvaluation}) 
& \mref{mParams}, \mref{mStats} \\

R5 (\textit{R-Scoring}) 
& \mref{mStats} \\

R6 (\textit{R-QuantifyGain}) 
& \mref{mStats}, \mref{mPlot} \\

NFR1 (\textit{Usability}) 
& \mref{mParams}, \mref{mPlot} \\
...
& \mref{mAudio}, \mref{mMEG}, \mref{mParams}, \mref{mPlot} \\
