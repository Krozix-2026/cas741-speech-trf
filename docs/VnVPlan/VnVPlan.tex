...
Output: System rejects the run and reports an \texttt{InvalidConfigurationError} (or equivalent validation failure) identifying the MEG sampling frequency field and stating that the value must be strictly positive. \\
...
\texttt{dropout} $\notin [0,1)$. \\
Output: System rejects the configuration and reports a validation error that identifies the invalid hyperparameter name and states the violated constraint (e.g., \texttt{num\_layers > 0}, \texttt{hidden\_size > 0}, or \texttt{dropout $\in$ [0,1)}). \\
Test Case Derivation: R2 \\
...
Output: System rejects training and extraction and reports a dimension-mismatch error identifying both the expected input dimension and the observed dimension. \\
Test Case Derivation: R2 \\
...
Output: System rejects the run and reports an alignment validation error identifying the predictor time axis and the MEG time grid as incompatible. \\
...
Output: System rejects scoring and reports a scoring error identifying the cause of invalidity, such as NaN values in the predictor matrix or undefined correlation due to a constant response signal. \\
